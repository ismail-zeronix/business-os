...
## Email quotations (approved and built 2026-09-21, stages A to C; stage D 2026-09-21 and 2026-09-23)
...
- **D.** Sending is audited on both the quotation and the customer (`quotation.emailed`, scope Customer), so it already shows on the customer's Activity tab. **Built 2026-09-23:** a Note / Call toggle above that tab logs a note (free text) or a call (direction, outcome, optional note) as an append-only audit entry (`customer.note_added`, `customer.call_logged`; same pattern as an enquiry note, no new table, no migration), so notes and calls now appear on the same timeline as emailed quotations and profile edits. The larger merged feed (enquiries, quotations, emails received matched by contact, filter chips, the same feed embedded on enquiry/quotation pages) stays in `docs/ideas/BACKLOG.md`.
...
**Verified 2026-09-23 (Stage D note/call logging).** Typecheck, ESLint, `next build` and the 142 existing tests clean. 13 checks in a rolled-back transaction: a note and two calls (with and without an optional note) write the right audit action and human-readable details, append rather than overwrite, and a missing customer id is rejected. Nothing was left in the project database (the check rolled back). **Not verified in a browser** — no browser automation tool was available in this session. Before treating this as done, open a customer, go to Activity, log a `TEST` note and a `TEST` call, and confirm both render correctly in the timeline with the right labels.

**Not built.** Merging the customer Activity tab with enquiries, quotations and emails received (the larger Customer Timeline idea), moving the enquiry to Quoted when a quotation is emailed, HTML email, replies and threading, open and click tracking (deliberately left out), sending anything other than the quotation, retry and a queue. See `docs/ideas/BACKLOG.md`.
